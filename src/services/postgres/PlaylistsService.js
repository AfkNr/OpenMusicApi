...
class PlaylistsService {
  ...
  async getPlaylists(owner) {
    try {
      ...
      const query = {
        text: 'SELECT playlists.id, playlists.name, users.username FROM playlists JOIN collaborations ON collaborations.playlist_id = playlists.id JOIN users ON collaborations.user_id = $1 WHERE playlists.owner = users.id ORDER BY playlists.id',
        values: [owner],
      };
      const result = await this._pool.query(query);
      return result.rows;
  ...
  async verifyPlaylistOwner(id, owner) {
    ...
    if (!result.rows.length) {
      throw new NotFoundError('Data yang Anda cari tidak ditemukan.');
    }

    if (result.rows[0].owner !== owner) {
      throw new AuthorizationError('Akses ke sumber daya ini tidak diizinkan');
    }
